export type FFTWindow = 'hann' | 'none';
...
export function getFFTPlan(size: number): FFTPlan {
  ...
  const plan: FFTPlan = { size: n, bitrev };
  planCache.set(n, plan);
  return plan;
}

export function applyWindowInPlace(re: Float32Array, window: FFTWindow): void {
  if (window === 'none') return;
  if (window !== 'hann') return;
...
export function fftComplexInPlace(re: Float32Array, im: Float32Array, plan?: FFTPlan): void {
  const n = re.length;
...
export function fftMagReal(
  input: Float32Array,
  fftSize: number,
  options: { window?: FFTWindow; plan?: FFTPlan; outMag?: Float32Array; outRe?: Float32Array; outIm?: Float32Array } = {}
): Float32Array {
