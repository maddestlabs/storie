...
export function createSimpleRegistry(defs: NodeDef[]): NodeRegistry {
  const map = new Map<string, NodeDef>();
  for (const def of defs) map.set(def.kind, def);
  return {
...
export function nodeById(graph: Graph): Map<string, GraphNode> {
...
function isTypeCompatible(src: ValueType, dst: ValueType): boolean {
  if (dst === 'any' || src === 'any') return true;
  if (src === dst) return true;

  // Allow number -> vector splat in some domains.
  if (src === 'number' && (dst === 'vec2' || dst === 'vec3' || dst === 'vec4' || dst === 'color')) return true;
  // Allow color <-> vec3/vec4 interchange (common in shaders).
  if (src === 'color' && (dst === 'vec3' || dst === 'vec4')) return true;
  if ((src === 'vec3' || src === 'vec4') && dst === 'color') return true;
...
export function computeLevels(graph: Graph, order?: string[]): Map<string, number> {
  const nodes = nodeById(graph);
  const ids = Array.from(nodes.keys());
  const out = new Map<string, string[]>();
...
export function validateGraph(graph: Graph, registry?: NodeRegistry): GraphValidationResult {
  ...
  const nodeBy = new Map<string, GraphNode>();
  ...
  const inputOccupied = new Set<string>();
