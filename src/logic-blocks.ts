import type { CodeBlock, LogicBlock, LogicStatement, Section } from './types.js';
...
function splitTopLevelComma(input: string): string[] | null {
  const parts: string[] = [];
  let start = 0;
  let quote: '"' | "'" | null = null;
  let escape = false;
...
function parseLogicStatement(line: string, lineNumber: number): LogicStatement | null {
  const trimmed = line.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith('//') || trimmed.startsWith('#')) return null;

  let relationMeta: Record<string, any> | null = null;
...
function findSectionForLine(sections: Section[], line: number): Section | null {
...
export function extractLogicBlocks(codeBlocks: CodeBlock[], sections: Section[]): LogicBlock[] {
  ...
  for (const block of codeBlocks) {
    ...
    const statements = block.code
      ...
      .filter((statement): statement is LogicStatement => !!statement);
...
export function attachLogicBlocksToSections(sections: Section[], logicBlocks: LogicBlock[]): Section[] {
  if (!logicBlocks.length) return sections;

  const bySectionId = new Map<string, LogicBlock[]>();
  ...
  const assign = (list: Section[]) => {
    for (const section of list) {
      ...
      if (section.children.length > 0) assign(section.children);
    }
  };
