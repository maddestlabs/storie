...
export type EaseSpec = EaseName | { type: 'cubicBezier'; x1: number; y1: number; x2: number; y2: number };

export type AutomationValue = number;

export type AutomationVarEvent =
  | { var: string; value: AutomationValue; ease?: EaseSpec }
  | { var: string; to: AutomationValue; durMs: number; ease?: EaseSpec };

export type AutomationCallEvent = { call: string; args?: any[] };

export type AutomationInputEvent = { input: { type: string; [k: string]: any } };

export type AutomationImpulseEvent =
  | ({ type: 'call' } & AutomationCallEvent & { ms: number })
  | ({ type: 'input' } & AutomationInputEvent & { ms: number });
...
export type CompiledAutomation = {
  /**
   * Map of varName -> eased segments.
   * Sampling is deterministic and depends only on time.
   */
  vars: Record<string, AutomationSegment[]>;
  /**
   * Impulse events that should fire on a forward-moving edge.
   */
  impulses: AutomationImpulseEvent[];
};

export type TimedEntryLike = { ms: number; text: string };
...
export function parseEaseSpec(raw: any): EaseSpec {
  if (!raw) return 'linear';
  if (typeof raw === 'string') {
    const s = raw.trim();
    const name = s as EaseName;
    return name;
  }
...
export function ease(u: number, spec: EaseSpec = 'linear'): number {
  const t = clamp01(Number.isFinite(u) ? u : 0);

  const named = (name: EaseName): number => {
    switch (name) {
  ...
  if (typeof spec === 'string') return named(spec as EaseName);
  ...
  if (isRecord(spec) && spec.type === 'cubicBezier') {
    ...
    const sampleDX = (tt: number) => (3 * ax * tt + 2 * bx) * tt + cx;
...
function valueAtSegments(segments: AutomationSegment[], tMs: number, defaultValue: number): number {
...
function pushSegment(segments: AutomationSegment[], seg: AutomationSegment): void {
...
export function compileAutomation(entries: TimedEntryLike[]): CompiledAutomation {
  const vars: Record<string, AutomationSegment[]> = {};
  const impulses: AutomationImpulseEvent[] = [];
  ...
  const currentTimeByVar: Record<string, number> = {};
  const currentValueByVar: Record<string, number> = {};
...
export function impulsesBetween(
  compiled: CompiledAutomation,
  prevTimeSec: number,
  nowTimeSec: number
): AutomationImpulseEvent[] {
  ...
  const out: AutomationImpulseEvent[] = [];
