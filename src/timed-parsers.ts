 ...
 */
export function parseTTMLTime(expr: string): number {
  const s = expr.trim();
  if (!s) return NaN;
  ...
  if (metricMatch) {
    ...
    if (unit === 't')  return Math.round(val / 10000); // ticks at 10MHz SMPTE
    if (unit === 'h')  return Math.round(val * 3600000);
    if (unit === 'm')  return Math.round(val * 60000);
    if (unit === 'f')  return Math.round((val / 30) * 1000); // assume 30fps fallback
  }
...
function finish(entries: TimedEntry[]): TimedEntry[] {
  return entries
 ...
 */
export function parseTimedNative(text: string): TimedEntry[] {
 ...
 */
export function parseSRT(text: string): TimedEntry[] {
  const entries: TimedEntry[] = [];
 ...
 */
export function parseWebVTT(text: string): TimedEntry[] {
  const entries: TimedEntry[] = [];
 ...
 */
export function parseTTML(text: string): TimedEntry[] {
  const entries: TimedEntry[] = [];
  ...
  function processBlock(htmlFragment: string, divOffsetMs: number) {
    pRe.lastIndex = 0;
    ...
    while ((pm = pRe.exec(htmlFragment)) !== null) {
      ...
      if (raw) entries.push({ ms, text: raw });
    }
  }
 ...
 */
export function parseWhisperJSON(obj: Record<string, any>): TimedEntry[] {
  const entries: TimedEntry[] = [];
  const segments = obj['segments'];
  if (!Array.isArray(segments)) return entries;
 ...
 */
function groupWords(words: Array<Record<string, any>>, startKey: string, wordKey: string, msScale: number): TimedEntry[] {
  const entries: TimedEntry[] = [];
  ...
  for (const w of words) {
    ...
    if (!lineWords.length) {
      ...
      (startMs - prevEnd * msScale > WORD_BREAK_GAP_MS) ||
 ...
 */
export function parseASRJSON(obj: Record<string, any>): TimedEntry[] {
  // 1. Whisper segments
  ...
  if (Array.isArray(obj['utterances'])) {
    ...
    for (const u of obj['utterances']) {
      ...
      if (Number.isFinite(ms) && t) entries.push({ ms, text: t });
    }
    if (entries.length) return finish(entries);
  }
  ...
  if (Array.isArray(obj['words']) && obj['words'].length) {
    ...
    const result     = groupWords(obj['words'] as Array<Record<string, any>>, 'start', 'text', msScale);
  ...
  if (Array.isArray(obj['monologues'])) {
    const entries: TimedEntry[] = [];
    for (const mono of obj['monologues'] as Array<Record<string, any>>) {
      const elems = mono['elements'];
      if (!Array.isArray(elems)) continue;
      const words = elems.filter((e: any) => e['type'] === 'text' || !e['type']);
  ...
  if (Array.isArray(obj['results'])) {
    const entries: TimedEntry[] = [];
    for (const res of obj['results'] as Array<Record<string, any>>) {
      ...
      if (Array.isArray(alt['words'])) {
        ...
        const words = (alt['words'] as Array<Record<string, any>>).map((w: any) => ({
  ...
  if (Array.isArray(obj['recognizedPhrases'])) {
    const entries: TimedEntry[] = [];
    for (const p of obj['recognizedPhrases'] as Array<Record<string, any>>) {
      ...
      const t = (Array.isArray(nBest) && nBest.length)
                ...
                : String(p['text'] ?? '');
      if (t.trim()) entries.push({ ms, text: t.trim() });
    }
    if (entries.length) return finish(entries);
  }
  ...
  try {
    ...
    if (chan) {
      const sentences = chan['paragraphs']?.['paragraphs']
        ?.flatMap((p: any) => p['sentences'] ?? []) as Array<any>;
      if (Array.isArray(sentences) && sentences.length) {
        const entries = sentences.map((s: any) => ({
      ...
      if (Array.isArray(chan['words'])) {
        const result = groupWords(chan['words'] as Array<Record<string, any>>, 'start', 'word', 1000);
        if (result.length) return finish(result);
      }
    }
  } catch { /* ignore */ }
  ...
  if (Array.isArray(obj)) {
    const arr = obj as Array<Record<string, any>>;
    const entries: TimedEntry[] = [];
 ...
 */
export function parseTimedJSON(text: string): TimedEntry[] {
  let obj: unknown;
  ...
  if ('segments' in obj) return parseWhisperJSON(obj as Record<string, any>);

  return parseASRJSON(obj as Record<string, any>);
}

// ── Format auto-detection ────────────────────────────────────────────────────

export type TimedFormat = 'native' | 'srt' | 'vtt' | 'ttml' | 'whisper' | 'json' | 'auto';

/**
 * Detect the format of a timed-text string from its content.
 * Returns a canonical format name or 'native' as the fallback.
 */
export function detectTimedFormat(text: string): Exclude<TimedFormat, 'auto'> {
 ...
 */
export function parseTimedFormat(text: string, format: TimedFormat = 'auto'): TimedEntry[] {
