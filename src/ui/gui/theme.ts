import { ColorUtils, type Color, type NamedStyle } from '../../types.js';
import type { MarkdownStyle } from '../document/types.js';

export interface GUIThemeDefaults {
  label: {
    fg: Color;
    bg: Color;
  };
...
export type GUIMarkdownThemeDefaults = Partial<MarkdownStyle>;

type NamedStyleLike = NamedStyle | null | undefined;

const TRANSPARENT = ColorUtils.rgba(0, 0, 0, 0);

const FALLBACKS: GUIThemeDefaults = {
  label: {
...
const MARKDOWN_FALLBACKS: MarkdownStyle = {
...
function styleOrFallback(style: NamedStyleLike, fallback: NamedStyleLike): NamedStyle {
  return (style ?? fallback ?? {}) as NamedStyle;
}

function pickFg(style: NamedStyleLike, fallback: Color): Color {
  return style?.fg ?? fallback;
}

function pickBg(style: NamedStyleLike, fallback: Color): Color {
  return style?.bg ?? fallback;
}

export function createGUIThemeDefaultsFromStyles(getStyle: (name: string) => NamedStyleLike): GUIThemeDefaults {
...
export function createGUIMarkdownThemeDefaultsFromStyles(getStyle: (name: string) => NamedStyleLike): GUIMarkdownThemeDefaults {
...
export function createDefaultGUIThemeDefaults(): GUIThemeDefaults {
  return {
...
export function createDefaultGUIMarkdownThemeDefaults(): GUIMarkdownThemeDefaults {
