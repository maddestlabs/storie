 ...
 */
import { mat4FromTransform, getCameraViewMatrix, getCameraProjectionMatrix, mat4Multiply } from './worlds.js';
...
export class WorldsRenderer {
    ...
    indexBuffer = null;
    uniformBuffer = null;
    lineUniformBuffer = null;
    lineVertexBuffer = null;
    lineIndexBuffer = null;
    sampler = null;
    uniformStride = 256;
    uniformCapacity = 0;
    lineUniformStride = 256;
    lineUniformCapacity = 0;
    static LINE_SEGMENTS = 16;
    backgroundTexture = null;
    backgroundShaderTexture = null;
    backgroundShaderMipLevelCount = 1;
    backgroundImageTexture = null;
    backgroundImageSource = null;
    sectionArtTextureCache = new Map();
    // Neutral 1x1 fallback used to ensure binding(3) is always valid.
    neutralBackgroundTexture = null;
    ...
    ensureNeutralBackgroundTexture() {
        if (this.neutralBackgroundTexture)
            return;
        ...
        this.device.queue.writeTexture({ texture: this.neutralBackgroundTexture }, new Uint8Array([128, 128, 128, 255]), { bytesPerRow: 4 }, { width: 1, height: 1 });
    }
    /**
    ...
    async init() {
        ...
        this.device.queue.writeTexture({ texture: this.backgroundTexture }, new Uint8Array([0, 0, 0, 0]), { bytesPerRow: 4 }, { width: 1, height: 1 });
    ...
    requestBuiltinShaderLoad(shaderName) {
        ...
        if (this.shaderManager.hasShader(name))
            return;
        if (this.loadingBuiltinShaders.has(name))
            return;
    ...
    setBackgroundImage(image) {
        if (this.backgroundImageSource === image)
            return;
    ...
    ensureLineUniformBufferCapacity(count) {
        ...
        if (this.lineUniformBuffer && this.lineUniformCapacity >= needed)
            return;
    ...
    createBindGroupForLine(uniformOffset) {
        if (!this.linePipeline || !this.lineUniformBuffer)
            return null;
     ...
     */
    render(camera, layouts, hoveredSectionIndex = null, background, connectors = [], sectionArt = new Map()) {
        ...
        const contentDistortStrength = paperEnabled
            ? (Number.isFinite(background.contentDistortStrength)
                ? Math.max(0, Math.min(0.05, background.contentDistortStrength))
                : 0)
            : 0;
        ...
        if (paperEnabled) {
            if (!this.backgroundTexture) {
                ...
                const screenLockRaw = background?.screenLock ?? background.shaderUniforms?.screenLock;
                const screenLock = typeof screenLockRaw === 'boolean'
                    ? screenLockRaw
                    : (Number.isFinite(screenLockRaw) ? screenLockRaw > 0.5 : false);
                ...
                const planeZOverride = background && Number.isFinite(background.paperPlaneZ)
                    ? background.paperPlaneZ
                    : null;
        ...
        const drawSectionQuad = (options) => {
            ...
            if (isCulled(mvpMatrix))
                return false;
            this.device.queue.writeBuffer(uniformBuffer, options.uniformOffset + 0, mvpMatrix.buffer, mvpMatrix.byteOffset, mvpMatrix.byteLength);
            const rect = options.highlightRect;
            const highlightEnabled = rect ? 1.0 : 0.0;
            this.device.queue.writeBuffer(uniformBuffer, options.uniformOffset + 64, new Float32Array([options.logicalWidth, options.logicalHeight, options.hover, highlightEnabled]));
            ...
            this.device.queue.writeBuffer(uniformBuffer, options.uniformOffset + 80, rect
        ...
        for (let i = 0; i < layouts.length; i++) {
            ...
            if (!layout.visible || !layout.texture)
                continue;
        ...
        if (connectors.length > 0 && this.linePipeline && this.lineUniformBuffer && this.lineVertexBuffer && this.lineIndexBuffer) {
            ...
            for (let i = 0; i < connectors.length; i++) {
                ...
                this.device.queue.writeBuffer(this.lineUniformBuffer, uniformOffset + 0, viewProjectionMatrix.buffer, viewProjectionMatrix.byteOffset, viewProjectionMatrix.byteLength);
    ...
    createBindGroupForTexture(texture, uniformOffset, backgroundDetailTexture) {
        if (!this.sharedBindGroupLayout || !this.uniformBuffer || !this.sampler)
            return null;
...
//# sourceMappingURL=worlds-renderer.js.map