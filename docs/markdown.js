...
import { parseTimedFormat, parseTimedFrames } from './timed-parsers.js';
...
export async function parseMarkdown(source) {
    ...
    if (blobBlocks.length > 0 && blobBlocks.some(b => !!b.magic)) {
        blobBlocks = await Promise.all(blobBlocks.map(async (b) => {
            if (!b.magic)
                return b;
...
function serializeHeadingDirective(section) {
    const directive = section.directive ? { ...section.directive } : {};
...
function serializeSectionTree(sections) {
    const parts = [];
    const visit = (section) => {
...
export function serializeMarkdownDocumentSource(document) {
...
function _parseTimedMs(v) {
    ...
    if (msMatch)
        return Math.max(0, parseFloat(msMatch[1]));
    // "4.5s"
    const sMatch = s.match(/^(\d+(?:\.\d+)?)\s*s$/);
    if (sMatch)
        return Math.max(0, parseFloat(sMatch[1]) * 1000);
    // bare number string
...
function _parseLooseDirectiveKey(raw) {
    ...
    if (key.startsWith("'"))
        return _parseSingleQuotedString(key);
...
export function parseHeadingDirectiveObject(raw) {
    const trimmed = raw.trim();
    if (!trimmed.startsWith('{') || !trimmed.endsWith('}'))
        return null;
...
function extractTimedBlocks(codeBlocks) {
    ...
    for (const block of codeBlocks) {
        ...
        const formatHint = (String(block.metadata?.["format"] ?? "auto").trim().toLowerCase());
        const entries = parseTimedFormat(block.code, formatHint);
...
function extractBlobBlocks(codeBlocks) {
    ...
    for (const block of codeBlocks) {
        ...
        const encoding = String(block.metadata?.enc ?? 'base64').trim().toLowerCase();
