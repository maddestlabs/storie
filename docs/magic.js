 ...
 */
export async function decompressString(base64Input) {
    try {
 ...
 */
export function parseMagicParams(paramString) {
    const result = {};
    let i = 0;
    while (i < paramString.length) {
        ...
        if (i >= paramString.length || key.length === 0)
            break;
 ...
 */
export function extractDeclaredParams(content) {
    const result = [];
 ...
 */
export function substituteMagicParams(content, params, syntax = '{{PARAM}}') {
    let result = content;
 ...
 */
export async function expandMagicBlocks(source) {
