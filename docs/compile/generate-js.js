import { CAPABILITY_RUNTIME_PACK_IMPORTS, } from '../runtime/capability-api.js';
...
function generateCapabilitiesModule(manifest) {
...
function generateRuntimePacksModule(manifest) {
    ...
    for (const capability of manifest.capabilityPacks) {
        for (const specifier of CAPABILITY_RUNTIME_PACK_IMPORTS[capability] ?? []) {
            if (importBindings.has(specifier))
                continue;
    ...
    const runtimePackModuleLines = manifest.capabilityPacks.map((capability) => {
        const bindings = (CAPABILITY_RUNTIME_PACK_IMPORTS[capability] ?? [])
            ...
            .filter((bindingName) => typeof bindingName === 'string');
...
export function generateCompileScaffold(app, manifest) {
...
//# sourceMappingURL=generate-js.js.map