...
function readSeedFromUrl() {
  try {
    // SES may or may not allow access to location.
...
function setSeedInUrl(seed) {
  try {
    ...
    if (!loc || !hist || typeof URL === 'undefined') return;

    const url = new URL(loc.href);
    url.searchParams.set('seed', String(seed));
    hist.replaceState(null, '', url.toString());
  } catch {
    // ignore (SES may block history/location)
    console.log("ARGGGGGHHHHHHH! NO ALLOWED!!!!!");
  }
}
