export function normalizeDecorativeBorderSpec(value) {
...
function resolveEdgeMode(edgeMode, side) {
    if (edgeMode === 'tile' || edgeMode === 'stretch')
        return edgeMode;
...
export function drawDecorativeBorder(ctx, image, spec, width, height) {
    if (spec.kind !== 'image9')
        return false;
    ...
    if (!(sourceLeft > 0 || sourceRight > 0 || sourceTop > 0 || sourceBottom > 0))
        return false;
    ...
    try {
        ...
        drawHorizontalEdge(ctx, image, { x: leftCut, y: 0, width: edgeSourceWidth, height: sourceTop }, { x: x1, y: y0, width: Math.max(0, x2 - x1), height: topHeight }, resolveEdgeMode(spec.edgeMode, 'top'), scale);
        drawHorizontalEdge(ctx, image, { x: leftCut, y: bottomCut, width: edgeSourceWidth, height: sourceBottom }, { x: x1, y: y2, width: Math.max(0, x2 - x1), height: bottomHeight }, resolveEdgeMode(spec.edgeMode, 'bottom'), scale);
        drawVerticalEdge(ctx, image, { x: 0, y: topCut, width: sourceLeft, height: edgeSourceHeight }, { x: x0, y: y1, width: leftWidth, height: Math.max(0, y2 - y1) }, resolveEdgeMode(spec.edgeMode, 'left'), scale);
        drawVerticalEdge(ctx, image, { x: rightCut, y: topCut, width: sourceRight, height: edgeSourceHeight }, { x: x2, y: y1, width: rightWidth, height: Math.max(0, y2 - y1) }, resolveEdgeMode(spec.edgeMode, 'right'), scale);
    }
    finally {
...
//# sourceMappingURL=decorative-borders.js.map