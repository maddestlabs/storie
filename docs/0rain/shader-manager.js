...
export class ShaderManager {
    ...
    async init() {
        ...
        this.device.queue.writeTexture({ texture: this.defaultMaterialTexture }, new Uint8Array([128, 255, 0, 0]), // roughness≈0.5, normalScale≈1.0, metallic=0, emissive=0
        ...
        this.device.queue.writeTexture({ texture: this.defaultBlueNoiseTexture }, blueNoiseData, { bytesPerRow: blueNoiseSize * 4 }, { width: blueNoiseSize, height: blueNoiseSize });
     ...
     */
    async ensureBuiltinShader(name, baseUrl = resolveBuiltinShaderBaseUrl()) {
        const shaderName = String(name ?? '').trim();
        if (!shaderName)
            return false;
        if (this.hasShader(shaderName))
            return true;
