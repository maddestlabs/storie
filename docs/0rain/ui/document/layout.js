...
function resolveInlineWidgetHeight(widget, charH) {
    if (widget.type === 'slider')
        return Math.max(Math.round(charH * 1.9), 30);
    if (widget.type === 'button')
        return Math.max(Math.round(charH * 1.65), 28);
    if (widget.type === 'checkbox')
        return Math.max(Math.round(charH * 1.4), 24);
...
function getRunWidth(run, maxWidthPx, measure, charW, charH) {
    if (run.kind === 'newline')
        return 0;
    if (run.kind === 'widget')
        return resolveInlineWidgetWidth(run.widget, charW, charH, maxWidthPx, measure);
    return measure ? measure(run.text) : run.text.length * charW;
}
function getRunHeight(run, charH) {
    if (run.kind === 'widget')
        return resolveInlineWidgetHeight(run.widget, charH);
    return charH;
}
function wrapRuns(runs, maxChars, charW, charH) {
    ...
    const pushLine = () => {
        while (isWhitespaceRun(current[0]))
            current.shift();
        while (isWhitespaceRun(current[current.length - 1]))
            current.pop();
...
function resolveAlignedX(baseX, availableWidth, contentWidth, align) {
    if (align === 'center')
        return baseX + Math.max(0, Math.round((availableWidth - contentWidth) / 2));
    if (align === 'right')
        return baseX + Math.max(0, availableWidth - contentWidth);
    return baseX;
}
function getCalloutToneColor(tone, style) {
    if (tone === 'note' || tone === 'info')
        return style.infoFg;
    if (tone === 'tip' || tone === 'important')
        return style.successFg;
    if (tone === 'warning')
        return style.warningFg;
    if (tone === 'caution')
        return style.errorFg;
    return style.infoFg;
}
...
function resolveWidgetHeight(widget, charH) {
    if (widget.type === 'slider')
        return Math.max(Math.round(charH * 3.4), 56);
    if (widget.type === 'button')
        return Math.max(Math.round(charH * 2.3), 38);
    if (widget.type === 'checkbox')
        return Math.max(Math.round(charH * 2), 32);
...
export function layoutMarkdownDocument(nodes, box, metrics, style, scrollY = 0, padding = 10, options) {
    const ops = [];
    const linkRegions = [];
    ...
    const renderNodes = (nodeList, contentX, contentWidth) => {
        ...
        for (const node of nodeList) {
            ...
            if (node.kind === 'list') {
                ...
                const gapPx = Number.isFinite(style.listMarkerGapPx)
                    ? Math.max(0, style.listMarkerGapPx)
                    : undefined;
                ...
                for (let itemIndex = 0; itemIndex < node.items.length; itemIndex++) {
                    ...
                    const hangIndentPx = Number.isFinite(style.listHangIndentPx)
                        ? Math.max(0, style.listHangIndentPx)
                        : markerAdvance;
            ...
            if (node.kind === 'image') {
                ...
                const imageH = Math.max(charH * 6, Math.min(Math.round(imageW * 1.25), Math.round(imageW * aspect)));
            ...
            if (node.kind === 'callout') {
                ...
                ops.splice(insertIndex, 0, { kind: 'rect', x: calloutX, y: calloutY, w: calloutW, h: calloutH, color: backgroundColor }, { kind: 'rect', x: calloutX, y: calloutY, w: barW, h: calloutH, color: accent });
