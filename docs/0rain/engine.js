...
import { parseTimedFormat } from './timed-parsers.js';
import { compileWorldsTimeline, stateAtWorldsTimeline, getWorldsTimelineSelectorKey, } from './worlds-timeline.js';
...
import { createCamera3D, updateCamera3D, createSection3DLayouts, getDefaultWorldsConfig, focusOnSection, focusOnSectionFit, setCameraTarget, getCameraViewMatrix, getCameraProjectionMatrix, mat4Multiply, mat4Invert, mat4FromTransform, mat4TransformVec4, mat4TransformPoint, mat4TransformDirection, vec3Dot, vec3Cross, vec3Normalize, vec3Sub, vec3Add, vec3Scale, vec3Length } from './worlds.js';
import { getWorldsPreset, listWorldsPresetNames } from './worlds-presets.js';
import { KEY } from './types.js';
import { ColorUtils } from './types.js';
import { drawDecorativeBorder, normalizeDecorativeBorderSpec, } from './decorative-borders.js';
import { decodeRenderableImageFromBytes, loadRenderableImageFromResolvedUrl, resolveRenderableImageUrl, } from './renderable-image.js';
import { HostSync, parseHostParams, createHostSessionIds, makeClientJoinUrl } from './host-sync.js';
import { DEFAULT_FONT_FALLBACK_STACK, buildFontStack, getPrimaryFontFamily, isProbablyMonospaceFontStack, tryLoadGoogleFontFamily } from './font-loading.js';
...
export class StorieEngine {
    ...
    // Security policy
    untrustedContent;
    allowCrossOriginDynamicImport;
    allowModuleResolverFromSandbox;
    renderer;
    compositor = null;
    sandbox;
    api; // User API (initialized in constructor)
    ...
    worldsCardFontStack = null;
    // Native browser APIs (shared instances)
    audioContext;
    audioUrlBufferCache = new Map();
    audioUrlInFlightCache = new Map();
    uiImageUrlCache = new Map(); // resolvedUrl -> registered imageId
    uiImageUrlInFlight = new Map();
    ...
    uiImagePending = new Map();
    backgroundImageUrlCache = new Map();
    backgroundImageUrlInFlightCache = new Map();
    backgroundImageUrlFailures = new Set();
    worldsBackgroundCompositeCache = new Map();
    audioGestureUnlocked = false;
    trustedAudioGestureDepth = 0;
    pendingGestureAudioStarts = [];
    lastTouchEventAt = 0;
    canvas2DContext = null;
    offscreenCanvas2D = null;
    safeAreaProbeElement = null;
    webglContext = null;
    webgpuDevice = null;
    // WebGPU UI (optional)
    webgpuUIRenderer = null;
    sectionWebGPUUIRenderer = null;
    webgpuFeaturePack = null;
    webgpuFeaturePackPromise = null;
    uiDocumentPack = null;
    uiDocumentPackPromise = null;
    guiRuntimePack = null;
    guiRuntimePackPromise = null;
    guiRuntimeInstallPromise = null;
    guiRuntimeInstalled = false;
    installGUIAPIImpl = null;
    audioRuntimePack = null;
    audioRuntimePackPromise = null;
    audioRuntimeInstalled = false;
    audioRuntimeInstallPromise = null;
    textRuntimePack = null;
    textRuntimePackPromise = null;
    textRuntimeInstalled = false;
    textRuntimeInstallPromise = null;
    authoredToolsRuntimePack = null;
    authoredToolsRuntimePackPromise = null;
    authoredToolsRuntimeInstalled = false;
    authoredToolsRuntimeInstallPromise = null;
    worldsInitializationPromise = null;
    // Live section rendering: user on:render callbacks draw into 3D section textures.
    _liveSections = new Set();
    _liveRenderCtx = null;
    _liveUIOverride = null;
    ...
    section3DLayouts = [];
    pending3DCameraFocus = null;
    // Last focus request that was actually applied (used to re-frame on resize).
    lastApplied3DCameraFocus = null;
    worldsConfig = getDefaultWorldsConfig();
    worldsEnabled = false;
    worldsLayoutCallback = null;
    // 3D interaction state (hover + basic navigation controls)
    worldsControlsEnabled = false;
    ...
    worldsLinkKeyHandlingEnabled = true;
    ...
    pinchZoomLastCenterX = 0;
    pinchZoomLastCenterY = 0;
    multiTouchRotateActive = false;
    multiTouchRotateLastCentroidX = 0;
    multiTouchRotateLastCentroidY = 0;
    doubleTapLastTime = 0;
    doubleTapLastX = 0;
    doubleTapLastY = 0;
    freeFlyLeftPanActive = false;
    freeFlyLeftDragSectionIndex = null;
    freeFlyLeftLastX = 0;
    freeFlyLeftLastY = 0;
    // 3D link-centric interaction (canvas.nim parity)
    hovered3DLink = null;
    focused3DLink = null;
    current3DSectionId = null;
    current3DSectionIndex = null;
    selected3DSectionId = null;
    selected3DSectionIndex = null;
    // Worlds section visit state (per-document)
    worldsVisitedSectionIds = new Set();
    worldsRemovedSectionIds = new Set();
    worlds3DRenderedLinkOverlay = {
        enabled: false,
        section: null,
        internalOnly: true,
        thickness: 0.22,
    ...
    activated3DLinksQueue = [];
    // 3D section texture rasterization cache
    sectionTextureCache = new Map();
    sectionLinkRegionsCache = new Map();
    sectionWidgetPlacementsCache = new Map();
    ...
    sectionAnimEnterTimes = new Map();
    worldsInlineWidgetInstances = [];
    worldsInlineWidgetEventsQueue = [];
    worldsInlineWidgetValueState = new Map();
    worldsInlineWidgetConfigState = new Map();
    debugGuiLastInlineWidgetState = null;
    debugGuiLastOverlayRenderState = null;
    nextMarkdownImageId = 1;
    ...
    currentTheme;
    styleSheet;
    currentThemeLabel = 'neotopia';
    themeOverrideFromUrl = null;
    // Timing
    frameCount = 0;
    elapsedTime = 0;
    deltaTime = 0;
    _preExportState = null;
    _exportTimedBlockSelection = null;
    lastFrameTime = 0;
    running = false;
    ...
    worldsSectionOverridesByDocument = new Map();
    outlineCache = null;
    worldsSectionContentOverridesByDocument = new Map();
    worldsSectionStyleOverridesByDocument = new Map();
    // Worlds Overview (host-only)
    worldsOverviewEnabled = false;
    worldsOverviewSavedTransforms = null;
    pendingWorldsOverview = null;
    // Cache last computed auto-layout step sizes so we don't rewrite positions every frame.
    worldsAutoLayoutCache = null;
    worldsTimelineRuntimeState = new WeakMap();
    // Dropped-file handling (binary-safe)
    lastDroppedFile = null;
    dropHandlingCleanup = null;
    ...
    inputDispatchDepth = 0;
    maxDropBytes = 50 * 1024 * 1024;
    ...
    canvas;
    hiddenTextInput = null;
    hiddenTextInputSyncing = false;
    resetWorldsVisitState() {
        this.worldsVisitedSectionIds.clear();
        this.worldsRemovedSectionIds.clear();
    }
    applyWorldsHiddenUntilVisitedVisibility() {
    ...
    applyThemeColors(theme, label, source) {
        ...
        try {
            if (this.api) {
                ...
                this.api.themes?.getName && (this.api.themes.getName = () => this.currentThemeLabel);
                this.api.tui?.syncTheme?.();
                this.api.gui?.syncTheme?.();
            }
        }
        catch {
            // ignore
    ...
    isUrlFlagEnabled(name) {
        if (typeof window === 'undefined' || typeof URLSearchParams === 'undefined')
            return false;
        try {
            const urlParams = new URLSearchParams(window.location.search);
            if (name === 'debugGui' && urlParams.get('content') === '0rain')
                return true;
    ...
    debugGuiLogInlineWidgets(state, detail) {
        if (!this.isUrlFlagEnabled('debugGui'))
            return;
        const serialized = `${state}:${JSON.stringify(detail)}`;
        if (serialized === this.debugGuiLastInlineWidgetState)
            return;
        this.debugGuiLastInlineWidgetState = serialized;
        console.log('[debugGui][inlineWidgets]', state, detail);
    }
    debugGuiLogOverlayRender(state, detail) {
        if (!this.isUrlFlagEnabled('debugGui'))
            return;
        const serialized = `${state}:${JSON.stringify(detail)}`;
        if (serialized === this.debugGuiLastOverlayRenderState)
            return;
    ...
    setWorldsOverviewEnabled(enabled, options) {
        ...
        const padding = Number.isFinite(options?.padding ?? NaN) ? options.padding : 20;
        const depth = Number.isFinite(options?.depth ?? NaN) ? options.depth : this.worldsConfig.defaultDepth;
        const fill = Number.isFinite(options?.fill ?? NaN) ? options.fill : 0.9;
        const levels = options?.levels ?? 'any';
        ...
        const candidates = nodes
            ...
            if (!layout)
                return false;
            if (!includeHidden && layout.visible === false)
                return false;
            if (!includeNonNavigable && layout.navigable === false)
                return false;
            return true;
        });
        ...
        setCameraTarget(this.camera3D, { x: centerX, y: centerY, z: depth + distance }, { x: 0, y: 0, z: 0 });
    }
    constructor(canvas, config = {}) {
        ...
        console.log(`  Theme: ${this.themeOverrideFromUrl ? `${this.themeOverrideFromUrl.label} (url override)` : 'neotopia (default)'}`);
    ...
    getDocumentMarkdownImageCache(documentId) {
        ...
        if (!doc._markdownImageCache) {
            doc._markdownImageCache = new Map();
        }
    ...
    ensureRenderableImageLoaded(rawUrl) {
        ...
        if (!rawKey || this.backgroundImageUrlFailures.has(rawKey))
            return null;
        let resolvedUrl;
        try {
        ...
        if (this.backgroundImageUrlFailures.has(resolvedUrl))
            return null;
    ...
    composeWorldsBackgroundOverlay(cacheKey, baseImage, overlayImage, options) {
        ...
        if (!(width > 0 && height > 0 && overlayWidth > 0 && overlayHeight > 0))
            return null;
    ...
    getMarkdownImageSize(source, documentId) {
        ...
        if (!cached || !cached.image || cached.width <= 0 || cached.height <= 0)
            return null;
    ...
    ensureWebGPUUI() {
        ...
        if (this._liveUIOverride)
            return this._liveUIOverride;
        if (!(this.renderer instanceof WebGPURenderer))
            return null;
        if (!this.compositor)
            return null;
        if (this.webgpuUIRenderer)
            return this.webgpuUIRenderer;
    ...
    async loadWebGPUFeaturePack() {
        if (this.webgpuFeaturePack)
            return this.webgpuFeaturePack;
        if (this.webgpuFeaturePackPromise)
    ...
    getWebGPUFeaturePack() {
        return this.webgpuFeaturePack;
    }
    async loadUIDocumentPack() {
        if (this.uiDocumentPack)
            return this.uiDocumentPack;
        if (this.uiDocumentPackPromise)
    ...
    async loadGUIRuntimePack() {
        if (this.guiRuntimePack)
            return this.guiRuntimePack;
        if (this.guiRuntimePackPromise)
    ...
    async loadAudioRuntimePack() {
        if (this.audioRuntimePack)
            return this.audioRuntimePack;
        if (this.audioRuntimePackPromise)
    ...
    async loadTextRuntimePack() {
        if (this.textRuntimePack)
            return this.textRuntimePack;
        if (this.textRuntimePackPromise)
    ...
    async loadAuthoredToolsRuntimePack() {
        if (this.authoredToolsRuntimePack)
            return this.authoredToolsRuntimePack;
        if (this.authoredToolsRuntimePackPromise)
    ...
    getAuthoredToolsRuntimePack() {
        return this.authoredToolsRuntimePack;
    }
    requestTextRuntimePack() {
        if (this.textRuntimePack || this.textRuntimePackPromise)
            return;
        void this.loadTextRuntimePack().catch((error) => {
            console.warn('Failed to load text runtime pack:', error);
        });
    }
    async ensureTextRuntimeInstalled() {
        if (this.textRuntimeInstalled)
            return;
        if (this.textRuntimeInstallPromise)
    ...
    requestAuthoredToolsRuntimePack() {
        if (this.authoredToolsRuntimePack || this.authoredToolsRuntimePackPromise)
            return;
        void this.loadAuthoredToolsRuntimePack().catch((error) => {
            console.warn('Failed to load authored tools runtime pack:', error);
        });
    }
    async ensureAuthoredToolsRuntimeInstalled() {
        if (this.authoredToolsRuntimeInstalled)
            return;
        if (this.authoredToolsRuntimeInstallPromise)
    ...
    requestAudioRuntimePack() {
        if (this.audioRuntimePack || this.audioRuntimePackPromise)
            return;
        void this.loadAudioRuntimePack().catch((error) => {
            console.warn('Failed to load audio runtime pack:', error);
        });
    }
    async ensureAudioRuntimeInstalled() {
        if (this.audioRuntimeInstalled)
            return;
        if (this.audioRuntimeInstallPromise)
    ...
    requestGUIRuntimePack() {
        if (this.guiRuntimePack || this.guiRuntimePackPromise)
            return;
        void this.loadGUIRuntimePack().catch((error) => {
            console.warn('Failed to load GUI runtime pack:', error);
        });
    }
    createDeferredGUIAPI() {
        ...
        return new Proxy(state, {
            get: (target, prop) => {
                ...
                if (prop === '_sectionBindings')
                    return [];
                if (prop === 'getSystem')
                    return () => null;
                if (prop === 'isAutoInputEnabled' || prop === 'isAutoUpdateEnabled')
                    return () => false;
    ...
    buildGUIAPI(createGUIAPI) {
        ...
        }, (name) => this.getStyle(name), () => this.inputDispatchDepth > 0, () => {
            try {
        ...
        }, () => this.getCanvasViewportRectCss(), () => this.getSafeAreaInsetsCss(), () => this.getResolvedCurrent3DSectionIndex(), (selector) => this.resolve3DSectionIndex(selector));
    }
    async ensureGUIRuntimeInstalled() {
        if (this.guiRuntimeInstalled)
            return;
        if (this.guiRuntimeInstallPromise)
    ...
    requestWebGPUFeaturePack() {
        if (this.webgpuFeaturePack || this.webgpuFeaturePackPromise)
            return;
        void this.loadWebGPUFeaturePack().catch((error) => {
            console.warn('Failed to load WebGPU feature pack:', error);
        });
    }
    hasPendingDocumentShaders() {
    ...
    async ensureShaderSupport(device) {
    ...
    async ensureWorldsRendererInitialized(device) {
        if (this.worldsRenderer)
            return true;
        if (this.worldsInitializationPromise)
    ...
    requestWorldsRendererInitialization() {
        if (!this.worldsEnabled || this.worldsRenderer || this.worldsInitializationPromise)
            return;
        if (!(this.renderer instanceof WebGPURenderer))
            return;
        const device = this.renderer.getContext().getDevice();
    ...
    createUserAPI() {
        ...
        this.installGUIAPIImpl = deferredGUIAPI.__setImpl;
        const MAX_BLOB_BYTES = 8 * 1024 * 1024;
        const getAuthoredToolsPack = () => engine.getAuthoredToolsRuntimePack();
        ...
        const applyWorldsConfigDefaults = (config) => {
            ...
            if (config.sectionClickFocusEnabled !== undefined) {
                engine.worldsConfig.sectionClickFocusEnabled = !!config.sectionClickFocusEnabled;
            }
            if (config.sectionSizeUnits !== undefined) {
            ...
            if (config.sectionOverflow !== undefined) {
            ...
            if (config.sectionContentAlign !== undefined) {
            ...
            if (config.sectionTextAlign !== undefined) {
            ...
            if (config.sectionGuiMode !== undefined) {
            ...
            if (config.keepRotation !== undefined) {
                engine.worldsConfig.keepRotation = !!config.keepRotation;
            }
            if (config.straightenOnFocus !== undefined) {
                engine.worldsConfig.straightenOnFocus = !!config.straightenOnFocus;
            }
            if (config.screenSpaceRecenter !== undefined) {
                engine.worldsConfig.screenSpaceRecenter = !!config.screenSpaceRecenter;
            }
            if (config.screenSpaceRecenterIters !== undefined) {
                const v = Number(config.screenSpaceRecenterIters);
                if (Number.isFinite(v)) {
                    engine.worldsConfig.screenSpaceRecenterIters = Math.max(1, Math.min(12, Math.floor(v)));
                }
            }
            ...
            if (config.sectionBorder !== undefined) {
            ...
            if (config.sectionBackground !== undefined) {
            ...
            if (config.sectionForeground !== undefined) {
            ...
            if (config.sectionBackgroundPaperNoiseStrength !== undefined) {
                ...
                if (Number.isFinite(v)) {
                    engine.worldsConfig.sectionBackgroundPaperNoiseStrength = Math.max(0, Math.min(1, v));
                }
            }
            ...
            if (config.sectionListMarker !== undefined) {
            ...
            if (config.sectionListMarkerGapPx !== undefined) {
            ...
            if (config.sectionListHangIndentPx !== undefined) {
            ...
            if (config.sectionArrowNavigation !== undefined) {
                engine.worldsConfig.sectionArrowNavigation = !!config.sectionArrowNavigation;
            }
            if (config.sectionTextureCacheRadius !== undefined) {
                const v = Number(config.sectionTextureCacheRadius);
                engine.worldsConfig.sectionTextureCacheRadius = Number.isFinite(v) ? Math.max(0, Math.floor(v)) : undefined;
            }
            if (config.multiTouchRotateEnabled !== undefined) {
                engine.worldsConfig.multiTouchRotateEnabled = !!config.multiTouchRotateEnabled;
            }
            if (config.doubleTapResetEnabled !== undefined) {
                engine.worldsConfig.doubleTapResetEnabled = !!config.doubleTapResetEnabled;
            }
            if (config.doubleTapResetRotation !== undefined) {
                const rot = config.doubleTapResetRotation;
                if (rot === null) {
                    engine.worldsConfig.doubleTapResetRotation = undefined;
                }
                else if (typeof rot === 'object' && rot !== null) {
                    const rx = Number(rot.x);
                    const ry = Number(rot.y);
                    const rz = Number(rot.z);
                    if (Number.isFinite(rx) && Number.isFinite(ry) && Number.isFinite(rz)) {
                        engine.worldsConfig.doubleTapResetRotation = { x: rx, y: ry, z: rz };
                    }
                }
            }
            if (config.navigationConstraints !== undefined) {
                const nc = config.navigationConstraints;
                if (nc === null) {
                    engine.worldsConfig.navigationConstraints = undefined;
                }
                else if (typeof nc === 'object') {
                    const out = {};
                    const clampNum = (v) => { const n = Number(v); return Number.isFinite(n) ? n : undefined; };
                    if (nc.minX !== undefined)
                        out.minX = clampNum(nc.minX);
                    if (nc.maxX !== undefined)
                        out.maxX = clampNum(nc.maxX);
                    if (nc.minY !== undefined)
                        out.minY = clampNum(nc.minY);
                    if (nc.maxY !== undefined)
                        out.maxY = clampNum(nc.maxY);
                    if (nc.minZ !== undefined)
                        out.minZ = clampNum(nc.minZ);
                    if (nc.maxZ !== undefined)
                        out.maxZ = clampNum(nc.maxZ);
                    if (nc.dragAxis === 'x' || nc.dragAxis === 'y')
                        out.dragAxis = nc.dragAxis;
                    else if (nc.dragAxis === null || nc.dragAxis === 'none')
                        out.dragAxis = null;
        ...
        const estimateAudioBufferBytes = (buffer) => {
        ...
        const getStfxrBakedStore = (documentId) => {
            ...
            if (!doc)
                return null;
            if (!doc._stfxrBakedStore)
                doc._stfxrBakedStore = new Map();
        ...
        const sanitizePlayOptions = (options) => {
            ...
            if (Number.isFinite(v))
                out.volume = Math.max(0, Math.min(2, v));
            const w = Number(options?.when);
            if (Number.isFinite(w) && w >= 0)
                out.when = Math.min(60, w);
            const output = options?.output;
            if (output && typeof output === 'object' && typeof output.connect === 'function')
                out.output = output;
            return out;
        };
        const sanitizeVoiceOptions = (options) => {
            const out = sanitizePlayOptions(options);
            const a = Number(options?.attack);
            if (Number.isFinite(a) && a >= 0)
                out.attack = Math.min(10, a);
            const d = Number(options?.decay);
            if (Number.isFinite(d) && d >= 0)
                out.decay = Math.min(10, d);
            const s = Number(options?.sustain);
            if (Number.isFinite(s))
                out.sustain = Math.max(0, Math.min(1, s));
            const r = Number(options?.release);
            if (Number.isFinite(r) && r >= 0)
                out.release = Math.min(10, r);
            const pk = Number(options?.peak);
            if (Number.isFinite(pk) && pk >= 0)
                out.peak = Math.min(10, pk);
            const pitchParams = options?.pitchParams;
            if (typeof pitchParams === 'string')
                out.pitchParams = pitchParams;
            else if (Array.isArray(pitchParams))
                out.pitchParams = pitchParams.map((p) => String(p));
            const gateParam = options?.gateParam;
            if (typeof gateParam === 'string' && gateParam)
                out.gateParam = gateParam;
            if (options?.scheduleEvents !== undefined)
                out.scheduleEvents = !!options.scheduleEvents;
            if (options?.obeyStopAfter !== undefined)
                out.obeyStopAfter = !!options.obeyStopAfter;
            return out;
        };
        const getAudioPack = () => {
            const pack = engine.getAudioRuntimePack();
            if (!pack)
                engine.requestAudioRuntimePack();
            return pack;
        };
        const getTextPack = () => {
            const pack = engine.getTextRuntimePack();
            if (!pack)
                engine.requestTextRuntimePack();
            return pack;
        };
        const ensureFigletFont = (entry, fallbackName) => {
            if (entry.font)
                return entry.font;
        ...
        const playPresetInternal = (presetIn, seed, options) => {
            ...
            if (nodeCount > MAX_NODES || edgeCount > MAX_EDGES || eventCount > MAX_EVENTS) {
                console.warn(`[stfxr.playPreset] Refusing to play overly large preset (nodes=${nodeCount}, edges=${edgeCount}, events=${eventCount}).`);
                return { stop: () => { } };
            }
        ...
        const voicePresetInternal = (presetIn, seed, options) => {
            const audioPack = getAudioPack();
            if (!audioPack) {
                return {
                    params: {},
            ...
            const eventCount = Array.isArray(preset.events) ? preset.events.length : 0;
            if (nodeCount > MAX_NODES || edgeCount > MAX_EDGES || eventCount > MAX_EVENTS) {
                console.warn(`[stfxr.voicePreset] Refusing to create voice from overly large preset (nodes=${nodeCount}, edges=${edgeCount}, events=${eventCount}).`);
                return {
                    params: {},
                    setHz: () => { },
                    noteOn: () => { },
        ...
        const evictStfxrBakedIfNeeded = (store) => {
            ...
            for (const e of store.values())
                total += e.bytes;
            if (total <= MAX_STFXR_BAKED_BYTES)
                return;
        ...
        const loadSoundFromBlobCached = async (name, documentId) => {
            ...
            if (!cache)
                return await loadAudioFromBlobInternal(key, documentId);
        ...
        const portableAudioAssets = new Map();
        const portableAudioVoices = new Map();
        let nextPortableAudioAssetId = 1;
        let nextPortableAudioVoiceId = 1;
        const cloneAudioAssetHandle = (handle) => ({ ...handle });
        const cloneAudioVoiceHandle = (handle) => ({ ...handle });
        const registerPortableAudioAsset = (buffer, source, origin) => {
            const handle = {
        ...
        const resolvePortableAudioAsset = (handleOrId) => {
        ...
        const resolvePortableAudioVoice = (voiceOrId) => {
        ...
        const createPortableAudioVoice = (assetEntry, options = {}) => {
        ...
        return {
            ...
            doc: {
                ...
                logicForSection: (section) => {
                    ...
                    const resolved = section === 'current'
                        ...
                        : engine.resolveRuntimeSectionRef(section);
                    const wantedSectionId = resolved?.sectionId ?? null;
                    if (!wantedSectionId)
                        return [];
                    return store
                        .filter((block) => block.sectionId === wantedSectionId)
                        .flatMap((block) => Array.isArray(block.statements) ? block.statements.map((statement) => ({
                        source: String(statement.source ?? ''),
                        target: String(statement.target ?? ''),
                        ...(typeof statement.rel === 'string' && statement.rel ? { rel: statement.rel } : {}),
                        ...(statement.meta ? { meta: { ...statement.meta } } : {}),
            ...
            scene: {
                ...
                setRevealStep: (n) => {
                    if (engine.hostAudienceView)
                        return;
                    const next = Number.isFinite(n) ? Math.max(0, Math.floor(n)) : 0;
                    if (engine.sceneState.revealStep === next)
                        return;
                ...
                nextRevealStep: () => {
                    ...
                    if (engine.sceneState.revealStep === n)
                        return;
                ...
                resetRevealStep: () => {
                    if (engine.hostAudienceView)
                        return;
                    if (engine.sceneState.revealStep === 0)
                        return;
            ...
            (this.renderer instanceof WebGPURenderer
                ...
                }), () => this.layers.getActive().buffer, (name) => this.getStyle(name), () => this.inputDispatchDepth > 0),
                    ...
                    return {
                        ...
                        lines: (name) => {
                            ...
                            if (!entry)
                                return null;
                            if (!entry.lines)
                                entry.lines = String(entry.text ?? '').split(/\r?\n/);
                            return entry.lines;
                        }
                    };
                },
            ...
            stfxr: {
                forDocument: (documentId) => {
                    ...
                    return {
                        ...
                        voicePreset: (preset, seed, options) => {
            ...
            sys: {
                params: {
                    get: (name, defaultValue) => {
                        try {
                            ...
                            if (raw.trim() !== '' && Number.isFinite(asNum))
                                return asNum;
                ...
                automation: {
                    ...
                    entriesBetween: (entries, prevTimeSec, nowTimeSec) => {
                        try {
                            const pack = getAuthoredToolsPack();
                            if (!pack) {
                                engine.requestAuthoredToolsRuntimePack();
            ...
            audio: {
                asset: {
                    ...
                    fromBlob: async (name, documentId) => {
                ...
                setGain: (voiceOrId, gain) => {
                    const entry = resolvePortableAudioVoice(voiceOrId);
                    if (!entry || !Number.isFinite(gain))
                        return false;
                ...
                setPlaybackRate: (voiceOrId, playbackRate) => {
                    const entry = resolvePortableAudioVoice(voiceOrId);
                    if (!entry || !Number.isFinite(playbackRate))
                        return false;
                ...
                buffer: {
                    create: (channels, frameCount, sampleRate) => {
                ...
                ambient: {
                    createLayeredBed: (config) => {
                        ...
                        const clampFinite = (value, fallback, min) => {
                        ...
                        const createImpulseBuffer = (seconds, decay) => {
                        ...
                        convolver.buffer = impulseConfig?.buffer ?? createImpulseBuffer(clampFinite(impulseConfig?.seconds, 1.2, 0.001), clampFinite(impulseConfig?.decay, 2.5, 0.001));
                        ...
                        return {
                            ...
                            setLevel: (level, rampSeconds, lfoDepthTarget) => {
            ...
            canvas2d: {
                ...
                drawBorder: (spec, x, y, w, h) => {
                    const ctx = engine.ensureCanvas2D();
                    if (!ctx || !spec || typeof spec !== 'object')
                        return false;
                    if (spec.kind !== 'image9')
                        return false;
            ...
            ui: {
                pointer: {
                    ...
                    clicked: (button = 0) => {
                        if (!engine.input.isMouseClicked(button))
                            return false;
            ...
            compositor: {
                ...
                loadBuiltInEffect: async (effectName, shaderName) => {
                    ...
                    if (!effect)
                        throw new Error('[compositor.loadBuiltInEffect] Missing effectName');
            ...
            worlds: {
                ...
                widgets: {
                    ...
                    getValue: (id, section) => {
                        const resolved = section === undefined || section === null || section === 'current'
                            ...
                            : engine.resolve3DSectionIndex(section);
                    ...
                    configure: (id, patch, section) => {
                        ...
                        if (typeof patch.showValue === 'boolean')
                            nextPatch.showValue = patch.showValue;
                        if (typeof patch.min === 'number' && Number.isFinite(patch.min))
                            nextPatch.min = patch.min;
                        if (typeof patch.max === 'number' && Number.isFinite(patch.max))
                            nextPatch.max = patch.max;
                        if (typeof patch.step === 'number' && Number.isFinite(patch.step) && patch.step > 0)
                            nextPatch.step = patch.step;
                        if (typeof patch.fg === 'number' && Number.isFinite(patch.fg))
                            nextPatch.fg = patch.fg;
                        if (typeof patch.trackColor === 'number' && Number.isFinite(patch.trackColor))
                            nextPatch.trackColor = patch.trackColor;
                        if (typeof patch.knobColor === 'number' && Number.isFinite(patch.knobColor))
                            nextPatch.knobColor = patch.knobColor;
                        if (typeof patch.knobHoverColor === 'number' && Number.isFinite(patch.knobHoverColor))
                            nextPatch.knobHoverColor = patch.knobHoverColor;
                        ...
                        if (live?.kind === 'slider') {
                            if (typeof nextPatch.label === 'string')
                                live.widget.label = nextPatch.label;
                            if (typeof nextPatch.showValue === 'boolean')
                                live.widget.showValue = nextPatch.showValue;
                            if (typeof nextPatch.min === 'number')
                                live.widget.min = nextPatch.min;
                            if (typeof nextPatch.max === 'number')
                                live.widget.max = nextPatch.max;
                            if (typeof nextPatch.step === 'number')
                                live.widget.step = nextPatch.step;
                            if (typeof nextPatch.fg === 'number')
                                live.widget.sliderStyle.fg = nextPatch.fg;
                            if (typeof nextPatch.trackColor === 'number')
                                live.widget.sliderStyle.trackColor = nextPatch.trackColor;
                            if (typeof nextPatch.knobColor === 'number')
                                live.widget.sliderStyle.knobColor = nextPatch.knobColor;
                            if (typeof nextPatch.knobHoverColor === 'number')
                                live.widget.sliderStyle.knobHoverColor = nextPatch.knobHoverColor;
                ...
                nav: (() => {
                    ...
                    const levelsPredicate = (levels) => {
                        if (levels === undefined || levels === 'any')
                            return (_n) => true;
                        if (typeof levels === 'number' && Number.isFinite(levels))
                            return (n) => n.level === levels;
                        if (levels && typeof levels === 'object') {
                    ...
                    const list = (rule) => {
                        ...
                        if (scope === 'global') {
                            ...
                            if (rootIndex === null || rootIndex < 0 || rootIndex >= nodes.length)
                                return [];
                    ...
                    const goto = (index, rule) => {
                        ...
                        const mode = rule?.mode === 'focus' ? 'focus' : 'fit';
                        if (mode === 'focus') {
                            const distance = typeof rule?.distance === 'number' ? rule.distance : 80;
                            engine.request3DCameraFocus({ kind: 'focus', sectionIndex, distance });
                        }
                        else {
                            const fill = typeof rule?.fill === 'number' ? rule.fill : 0.9;
                ...
                camera: {
                    ...
                    shake: {
                        ...
                        setParams: (params) => {
                            ...
                            if (params?.translate && typeof params.translate === 'object') {
                                if (typeof params.translate.x === 'number' && Number.isFinite(params.translate.x))
                                    s.translate.x = params.translate.x;
                                if (typeof params.translate.y === 'number' && Number.isFinite(params.translate.y))
                                    s.translate.y = params.translate.y;
                                if (typeof params.translate.z === 'number' && Number.isFinite(params.translate.z))
                                    s.translate.z = params.translate.z;
                            }
                            if (params?.rotate && typeof params.rotate === 'object') {
                                if (typeof params.rotate.x === 'number' && Number.isFinite(params.rotate.x))
                                    s.rotate.x = params.rotate.x;
                                if (typeof params.rotate.y === 'number' && Number.isFinite(params.rotate.y))
                                    s.rotate.y = params.rotate.y;
                                if (typeof params.rotate.z === 'number' && Number.isFinite(params.rotate.z))
                                    s.rotate.z = params.rotate.z;
                            }
                        },
                        getParams: () => {
                            if (!engine.camera3D?.shake) {
                    ...
                    focusOnSection: (sectionIndex, distance = 50, options) => {
                        if (!engine.camera3D)
                            return;
                        const keepRotation = !!options?.keepRotation;
                ...
                _isLive: (idx) => engine._liveSections.has(idx),
                get _activeLiveSectionIndex() {
                ...
                content: {
                    ...
                    stateAt: (entries, timeSec, options) => {
                        return stateAtWorldsContent(entries, Number(timeSec) || 0, options);
                    },
                    applyTimed: (selector, entries, timeSec, options) => {
    ...
    async applyFrontmatterFontConfig(metadata) {
        ...
        const changed = nextSize !== this.fontSize ||
    ...
    renderWorldsSectionBoundGUI(baseUI) {
        ...
        const bindings = Array.isArray(guiAPI?._sectionBindings)
        ...
        for (const binding of bindings) {
            for (const sectionIndex of binding.sections) {
                const layout = this.getSectionLayoutByIndex(sectionIndex);
                if (!layout || !layout.visible || layout.interactive === false || !layout.texture)
                    continue;
    ...
    ensureWorldsSectionLayoutCaches(layout) {
        ...
        const fontStack = this.worldsCardFontStack ||
            this.fontFamily ||
        ...
        const overflowMode = (overflowCfg === 'expand' || overflowCfg === 'expand-y' || overflowCfg === 'fit' || overflowCfg === 'fit-y')
            ? overflowCfg
            : 'clip';
        ...
        if (overflowMode !== 'clip') {
            ...
            const probe = uiDocumentPack.layoutMarkdownDocument(nodes, { x: 0, y: 0, width: probeWidthPx, height: heightPx }, {
                charW: measuredCharW,
                charH: measuredCharH,
                measureTextWidth,
                getImageSize: (source) => this.getMarkdownImageSize(source, this.activeDocumentId ?? undefined),
            }, mdStyle, 0, texturePadding, { overflow: 'expand' });
        ...
        const result = uiDocumentPack.layoutMarkdownDocument(nodes, { x: 0, y: 0, width: widthPx, height: heightPx }, {
            charW: measuredCharW,
            charH: measuredCharH,
            measureTextWidth,
            getImageSize: (source) => this.getMarkdownImageSize(source, this.activeDocumentId ?? undefined),
        }, mdStyle, 0, texturePadding, this.getWorldsWidgetLayoutOptions(layout.sectionIndex, layoutOverflow));
    ...
    getActiveWorldsInlineWidgetPlacements() {
        ...
        if (!layout || !layout.visible || layout.interactive === false)
            return null;
        if (!this.ensureWorldsSectionLayoutCaches(layout))
            return null;
    ...
    projectWorldPointToScreen(point, options) {
        ...
        if (!clampToViewport && !allowOffscreen && (ndcX < -1 || ndcX > 1 || ndcY < -1 || ndcY > 1))
            return null;
    ...
    project3DTexturePointToScreen(layout, point, options) {
        ...
        return this.project3DCardLocalPointToScreen(layout, { x: u - 0.5, y: 0.5 - v, z: 0 }, options);
    }
    ...
    getVisual3DLinkConnections(options) {
        ...
        const out = [];
        for (const layout of this.section3DLayouts) {
            if (requestedSectionId && layout.sectionId !== requestedSectionId)
                continue;
            if (!layout.texture || !layout.visible || layout.interactive === false)
                continue;
    ...
    getRectAttachmentPointLocal(bounds, toward) {
        ...
        const ty = Math.abs(dy) > 1e-6
            ...
            : Number.POSITIVE_INFINITY;
        const t = Math.min(Number.isFinite(tx) && tx > 0 ? tx : Number.POSITIVE_INFINITY, Number.isFinite(ty) && ty > 0 ? ty : Number.POSITIVE_INFINITY);
    ...
    getCardAttachmentPointLocal(toward) {
        return this.getRectAttachmentPointLocal({ minX: -0.5, maxX: 0.5, minY: -0.5, maxY: 0.5, centerX: 0, centerY: 0 }, toward);
    }
    getRendered3DLinkConnectors() {
        if (!this.worlds3DRenderedLinkOverlay.enabled)
            return [];
        ...
        const addConnectorsForSource = (sourceLayout) => {
            if (!sourceLayout || !sourceLayout.visible || !sourceLayout.texture)
                return;
            ...
            for (let linkIndex = 0; linkIndex < regions.length; linkIndex++) {
                const region = regions[linkIndex];
                const internal = typeof region.url === 'string' && region.url.startsWith('#');
                if (this.worlds3DRenderedLinkOverlay.internalOnly && !internal)
                    continue;
                const targetLayout = internal ? this.resolveWorldsInternalLinkTarget(region.url) : null;
                if (!targetLayout || !targetLayout.visible || !targetLayout.texture)
                    continue;
    ...
    handleWorldsSectionBoundGUIMouse(pixelX, pixelY, mouseDown) {
        ...
        for (const candidate of candidates) {
            ...
            if (!dims)
                continue;
            if (local.x < 0 || local.y < 0 || local.x > dims.width || local.y > dims.height)
                continue;
    ...
    handleOverlayRetainedGUIMouse(pixelX, pixelY, mouseDown) {
        ...
        if (!system)
            return false;
        if (typeof guiAPI?.isAutoInputEnabled === 'function' && !guiAPI.isAutoInputEnabled())
            return false;
    ...
    handleOverlayRetainedGUIKey(key, modifiers) {
        ...
        if (!focused)
            return false;
        const excludedGroups = this.getWorldsSectionBoundGUIGroupIds(guiAPI);
        if (excludedGroups.has(focused.group))
            return false;
    ...
    handleWorldsInlineWidgetKey(key, modifiers) {
        const active = this.getActiveWorldsInlineWidgetPlacements();
        if (!active || active.placements.length === 0)
            return false;
        const guiAPI = this.api?.gui;
        const system = guiAPI?.getSystem?.();
        if (!system)
        ...
        if (!focusedIsInline && !navigatesInline)
            return false;
    ...
    async prepareDocumentForActivation(parsed) {
    ...
    buildDocumentAssetStores(parsed) {
        ...
        const stfxrStore = new Map();
        const stfxrPending = new Map();
        for (const b of parsed.codeBlocks) {
            if (b.lang !== 'stfxr')
                continue;
            const name = String(b.metadata?.name ?? '').trim();
        ...
        if (stfxrPending.size > 0) {
            ...
            for (let pass = 0; pass < maxPasses && stfxrPending.size > 0; pass++) {
                ...
                for (const [name, pending] of Array.from(stfxrPending.entries())) {
                    ...
                    if (Array.isArray(patch.edgesRemove) && patch.edgesRemove.length > 0) {
                        base.edges = (base.edges ?? []).filter(e => !patch.edgesRemove.some((r) => sameEdge(e, r)));
                    }
                    if (Array.isArray(patch.edgesAdd) && patch.edgesAdd.length > 0) {
                        const edges = [...(base.edges ?? [])];
                        for (const e of patch.edgesAdd) {
                            if (!edges.some(x => sameEdge(x, e)))
                                edges.push(e);
                        }
                        base.edges = edges;
                    }
            ...
            for (const pending of stfxrPending.values()) {
                console.warn(`  [stfxr] Could not resolve base "${pending.base}" for "${pending.name}" at lines ${pending.startLine + 1}-${pending.endLine + 1}`);
            }
        }
    ...
    storeLoadedDocument(documentId, parsed, handlers, assetStores, sourceMarkdown) {
    ...
    getActiveCompiledSectionContext() {
    ...
    async loadCompiledApp(documentId, compiledModule) {
        try {
            ...
            if (typeof runtime.export === 'function') {
                handlers.export = (options) => runtime.export?.(options ?? runtimeCtx());
            }
            scope.init = handlers.init;
            scope.update = handlers.update;
            scope.render = handlers.render;
            scope.input = handlers.input;
            scope.drop = handlers.drop;
            if (handlers.export) {
                scope.export = handlers.export;
            }
            const enterHandlers = {};
            let sectionIndex = 0;
            ...
            scope.__enterHandlers = enterHandlers;
    ...
    async loadMarkdown(documentId, markdown) {
        try {
            ...
            const jsBlocks = parsed.codeBlocks.filter(block => block.lang === 'javascript' || block.lang === 'js');
            ...
            const sectionIndexBySlug = new Map();
            {
                let idx = 0;
                const walk = (sections) => {
            ...
            const wrapWithSectionGuard = (hook, block) => {
                ...
                if (sectionMeta === undefined || sectionMeta === null)
                    return block.code;
                ...
                if (sectionIdx === null) {
                    console.warn(`  [section] Could not resolve section "${raw}" for on:${hook} at lines ${block.startLine + 1}-${block.endLine + 1}; running unscoped.`);
                    return block.code;
                }
            ...
            const enterBlocksBySection = new Map();
...
${exportVars}
            ...
            if (enterBlocksBySection.size > 0) {
                ...
                for (const idx of indices) {
                    ...
                    if (importVars)
                        pieces.push(importVars);
                    if (captureVars)
                        pieces.push(captureVars);
                    pieces.push(blocks.join('\n\n'));
                    if (exportVars)
                        pieces.push(exportVars);
                    pieces.push(`};`);
                }
    ...
    getActiveDocumentSourceMarkdown() {
        const doc = this.getActiveDocument();
        if (!doc)
            return null;
        if (typeof doc.sourceMarkdown === 'string')
            return doc.sourceMarkdown;
    ...
    getActiveWorldsSectionContentOverrideMap(create = false) {
    ...
    getActiveWorldsSectionStyleOverrideMap(create = false) {
        const documentId = this.activeDocumentId;
        if (!documentId)
            return null;
    ...
    clearWorldsSectionStyleOverride(selector) {
        ...
        if (!overrides)
            return false;
        if (!overrides.has(ref.sectionId))
            return false;
        overrides.delete(ref.sectionId);
        this.invalidate3DSectionTexture(ref.sectionIndex);
        return true;
    ...
    resolveThemeColorString(value) {
        ...
        switch (key) {
            ...
            case 'bg':
            case 'background': return this.currentTheme.bg;
            case 'bgalt':
            case 'bg_alt': return this.currentTheme.bgAlt;
            case 'fg':
            case 'foreground': return this.currentTheme.fg;
            case 'fgalt':
            case 'fg_alt': return this.currentTheme.fgAlt;
            case 'accent1':
            case 'primary': return this.currentTheme.accent1;
            case 'accent2':
            case 'secondary': return this.currentTheme.accent2;
            case 'accent3':
            case 'tertiary': return this.currentTheme.accent3;
            default: return null;
        }
    }
    applyWorldsTimedContent(selector, entries, timeSec, options) {
        const ref = this.resolveWorldsContentSectionRef(selector);
        if (!ref)
            return null;
    ...
    getActiveWorldsSectionOverrideMap(create = false) {
        const documentId = this.activeDocumentId;
        if (!documentId)
            return null;
    ...
    createRuntimeSectionFromInput(input, level = 1) {
        const normalizedLevel = this.clampRuntimeSectionLevel(Number.isFinite(input?.level) ? Number(input.level) : level);
        ...
        const section = {
            ...
            startLine: Number.isFinite(input?.startLine) ? Number(input.startLine) : -1,
            endLine: Number.isFinite(input?.endLine) ? Number(input.endLine) : -1,
            children: [],
        };
        if (Number.isFinite(input?.timedMs)) {
    ...
    insertRuntimeSection(sectionInput, options) {
        ...
        const nextSection = this.createRuntimeSectionFromInput(sectionInput, parentRef ? parentRef.section.level + 1 : (Number.isFinite(sectionInput.level) ? Number(sectionInput.level) : 1));
    ...
    updateRuntimeSection(selector, patch) {
        ...
        if (Array.isArray(patch.children)) {
            ref.section.children = patch.children.map((child) => this.createRuntimeSectionFromInput(child, ref.section.level + 1));
    ...
    applyWorldsTimeline(track, timeSec) {
        ...
        for (const selector of track.sections) {
            ...
            if (this.worldsTimelineStatesEqual(previous, desired))
                continue;
            const contentPatch = {};
            if (!previous || previous.title !== desired.title)
                contentPatch.title = desired.title;
            if (!previous || previous.content !== desired.content)
                contentPatch.content = desired.content;
            ...
            if (!previous ||
                ...
                if (activePatch?.position)
                    override.position = { ...desired.position };
                else
                    delete override.position;
                if (activePatch?.rotation)
                    override.rotationDegrees = { ...desired.rotation };
                else
                    delete override.rotationDegrees;
                if (activePatch?.scale)
                    override.scale = { ...desired.scale };
                else
                    delete override.scale;
            }
            runtime.lastAppliedByKey.set(key, desired);
        }
        return nextState;
            ...
            if (!success) {
                ...
                if (device) {
                    ...
                    if (this.pendingShaderChain && this.shaderChainManager) {
                        ...
                        await this.shaderChainManager.activateChainFromString(this.pendingShaderChain.chainStr, this.pendingShaderChain.source);
                        this.pendingShaderChain = null;
                    }
                }
            }
    ...
    runFrame() {
        try {
            ...
            if (this.compositor) {
                ...
                if (this.worldsEnabled && this.worldsRenderer && this.camera3D) {
                    ...
                    const textureImage = textureInfo && baseTextureImage
                        ? (overlayTextureImage && textureInfo.overlayUrl
                            ? this.composeWorldsBackgroundOverlay(`bg-overlay:${String(this.worldsConfig.sectionBackground ?? '')}`, baseTextureImage, overlayTextureImage, {
                    ...
                    let mergedShaderUniforms = shaderInfo?.uniforms
                        ? { ...shaderInfo.uniforms }
                    ...
                    const shaderCoordScaleRaw = mergedShaderUniforms
                        ? mergedShaderUniforms.coordScale
                        : undefined;
                    let shaderCoordScale = Number.isFinite(shaderCoordScaleRaw)
                        ? shaderCoordScaleRaw
                        : 1;
                    ...
                    if (shaderInfo && !(Number.isFinite(shaderCoordScaleRaw))) {
                        ...
                        if (shaderNameKey === 'ruledlines') {
                            const rowsRaw = mergedShaderUniforms ? mergedShaderUniforms.rowsPerTile : undefined;
                            const rows = Number.isFinite(rowsRaw) && rowsRaw > 0
                                ? rowsRaw
                                : 32;
                    ...
                    if (shaderInfo && mergedShaderUniforms) {
                        ...
                        if (shaderNameKey === 'ruledlines') {
                            ...
                            if (hasScale && !hasRows) {
                                ...
                                if (s > 0)
                                    mergedShaderUniforms.rowsPerTile = 1 / s;
                            }
                            else if (hasRows && !hasScale) {
                                const rows = mergedShaderUniforms.rowsPerTile;
                                if (rows > 0)
                                    mergedShaderUniforms.coordScale = 1 / rows;
                            }
                        }
                    }
                    const paperPlaneZ = (() => {
                        ...
                        if (planeZMode === 'focus') {
                            ...
                            if (!(typeof focusedIdx === 'number' && Number.isFinite(focusedIdx)))
                                return undefined;
                    ...
                    const textureTilePx = Number.isFinite(textureInfo?.tilePx)
                        ? Math.max(1, textureInfo.tilePx)
                        : 512;
                    const textureCoordScale = Number.isFinite(textureInfo?.coordScale)
                        ? textureInfo.coordScale
                    ...
                    const backgroundConfig = proceduralBackground || shaderInfo || textureInfo
                        ? {
                            ...
                            sectionBlendMode: (textureInfo?.blendMode ?? this.worldsConfig?.sectionBlendMode),
                            contentDistortStrength: (Number.isFinite(textureInfo?.contentDistort)
                                ? textureInfo.contentDistort
                                : (Number.isFinite(this.worldsConfig?.contentDistortStrength)
                                    ? this.worldsConfig.contentDistortStrength
                                    : 0)),
                            contentBlendStrength: Number.isFinite(textureInfo?.blendStrength)
                                ? textureInfo.blendStrength
                                : undefined,
                        }
                        : undefined;
                    const sectionArtStates = new Map();
                    for (const layout of this.section3DLayouts) {
                ...
                const guiSystemForOverlay = this.api?.gui?.getSystem?.();
                ...
                if (overlayRenderer) {
                    ...
                    let inlineGui = null;
                    if (this.worldsInlineWidgetInstances.length > 0) {
                        inlineGui = guiSystemForOverlay ?? this.api?.gui?.getSystem?.();
                    }
                    if (inlineGui) {
                        const { charWidth, charHeight } = this.getGUIPixelMetrics();
                    ...
                    if (systemForSectionGUI && this.worldsEnabled && this.camera3D && bindingsForSectionGUI.length > 0) {
                        ...
                        if (sectionGuiMode === 'baked') {
                            ...
                            const needsRebake = (typeof anySystem.needsRedraw === 'function' && !!anySystem.needsRedraw()) ||
    ...
    pauseForExport() {
        ...
        const doc = this.getActiveDocument();
        const handler = doc?.handlers?.export;
        if (handler) {
            try {
    ...
    _exportAudioBuffer = null;
    _exportAudioOffset = 0;
    /**
    ...
    bakeLiveSectionTexture(layout, device, baseLineHeight) {
        ...
        const liveMin = Number.isFinite(this.worldsConfig.liveTextureScale)
            ? Math.max(1, Math.min(4, this.worldsConfig.liveTextureScale))
            : 2;
    ...
    ensure3DSectionTextures(device) {
        ...
        for (const layout of this.section3DLayouts) {
            ...
            if (!layout.visible && !needsBakedGui)
                continue;
            ...
            if (layout.texture) {
                ...
                if (existing && existing.activeLinkIndex === activeLinkIndex) {
                    ...
                    if (layout.worldWidth !== prevW || layout.worldHeight !== prevH)
                        worldSizeChanged = true;
                    continue;
                }
            }
            const minW = 256;
            const minH = 128;
            const deviceMax = (device.limits && device.limits.maxTextureDimension2D)
                ? Number(device.limits.maxTextureDimension2D)
                : 2048;
            ...
            const result = uiDocumentPack.layoutMarkdownDocument(nodes, { x: 0, y: 0, width: widthPx, height: heightPx }, {
                ...
                measureTextWidth: this.worldsCardFontStack ? (text) => ctx.measureText(text).width : undefined,
                getImageSize: (source) => this.getMarkdownImageSize(source, this.activeDocumentId ?? undefined),
            }, mdStyle, 0, texturePadding, this.getWorldsWidgetLayoutOptions(layout.sectionIndex, layoutOverflow));
            ...
            let uploaded = false;
            try {
                device.queue.copyExternalImageToTexture({ source: canvas }, { texture }, { width: textureWidthPx, height: textureHeightPx });
                uploaded = true;
            }
            catch {
                ...
                try {
                    if (isOffscreenCanvas && typeof canvas.transferToImageBitmap === 'function') {
                        ...
                        device.queue.copyExternalImageToTexture({ source: bitmap }, { texture }, { width: textureWidthPx, height: textureHeightPx });
                        bitmap.close();
                        uploaded = true;
                    }
                }
            ...
            if (!uploaded) {
                try {
                    ...
                    if (imageData && imageData.data) {
                        ...
                        device.queue.writeTexture({ texture }, padded, { bytesPerRow }, { width: textureWidthPx, height: textureHeightPx });
                        uploaded = true;
                    }
                }
                catch (error) {
     ...
     */
    evictDistantSectionTextures() {
        const radius = this.worldsConfig.sectionTextureCacheRadius;
        ...
        const candidates = this.section3DLayouts.filter((l) => l && l.navigable
        ...
        if (currentPos < 0)
            return;
        for (let i = 0; i < candidates.length; i++) {
            if (Math.abs(i - currentPos) <= radius)
                continue;
    ...
    reflowWorldsAutoLayout() {
        ...
        const baseStep = Number.isFinite(this.worldsConfig.autoLayoutSpacing ?? NaN)
            ? this.worldsConfig.autoLayoutSpacing
            : 200;
        const padding = 20;
        ...
        for (const l of this.section3DLayouts) {
            ...
            if (Math.abs(x - prevX) > 1e-4 || Math.abs(y - prevY) > 1e-4)
                anyMoved = true;
        }
    ...
    parseWorldsSectionBackgroundChain() {
        const v = this.worldsConfig.sectionBackground;
        if (typeof v !== 'string')
            return [];
    ...
    parseWorldsSectionBackgroundTexture() {
        const v = this.worldsConfig.sectionBackground;
        if (typeof v !== 'string' || !v.startsWith('texture:'))
            return null;
        ...
        for (const spec of paramSpecs) {
            ...
            if (trimmedKey === 'overlayFit') {
                ...
                if (lower === 'contain')
                    overlayFit = 'contain';
                else if (lower === 'stretch' || lower === 'fill')
                    overlayFit = 'stretch';
                else
                    overlayFit = 'cover';
                continue;
    ...
    resolveWorldsSectionForeground() {
        const v = this.worldsConfig.sectionForeground;
        if (v === undefined || v === null)
            return null;
        if (typeof v === 'number')
            return v;
        if (typeof v === 'string')
            return this.resolveThemeColorString(v);
        return ColorUtils.from(v);
    }
    ...
    resolveEffectiveSectionForeground(sectionId) {
        ...
        if (perSection?.fg !== undefined)
            return perSection.fg;
        ...
        for (const layout of this.section3DLayouts) {
            ...
            if (overflowMode === 'expand' || overflowMode === 'expand-y' || overflowMode === 'fit' || overflowMode === 'fit-y') {
                ...
                const probe = uiDocumentPack.layoutMarkdownDocument(nodes, { x: 0, y: 0, width: probeWidthPx, height: heightPx }, {
                    ...
                    charH,
                    getImageSize: (source) => this.getMarkdownImageSize(source, this.activeDocumentId ?? undefined),
                }, style, 0, texturePadding, { overflow: 'expand' });
                const reqW = Math.ceil(probe.contentWidth + texturePadding * 2);
                const reqH = Math.ceil(probe.contentHeight + texturePadding * 2);
                const clampedW = Math.max(minW, Math.min(maxW, reqW));
                const clampedH = Math.max(minH, Math.min(maxH, reqH));
            ...
            if (existing &&
                ...
                if (!this.sectionLinkRegionsCache.has(layout.sectionId) || !this.sectionWidgetPlacementsCache.has(layout.sectionId)) {
                    ...
                    const result = uiDocumentPack.layoutMarkdownDocument(nodes, { x: 0, y: 0, width: widthPx, height: heightPx }, {
                        ...
                        charH,
                        getImageSize: (source) => this.getMarkdownImageSize(source, this.activeDocumentId ?? undefined),
                    }, style, 0, texturePadding, this.getWorldsWidgetLayoutOptions(layout.sectionIndex, layoutOverflow));
                    this.applyWorldsContentAlignment(result, widthPx, heightPx, texturePadding, layout.contentAlign);
                    this.sectionLinkRegionsCache.set(layout.sectionId, this.scaleLinkRegions(result.linkRegions, textureScale));
                    this.sectionWidgetPlacementsCache.set(layout.sectionId, this.scaleWidgetPlacements(result.widgetPlacements, textureScale));
                }
            ...
            ui.clearCommands();
            if (bakedRuledPaper) {
                const ruledLine = this.withAlpha(style.mutedFg, 0x40);
        ...
        if (this.worldsEnabled && this.worldsControlsEnabled && this.camera3D) {
            ...
            if (rmbDown) {
                if (!this.mouseLookActive) {
                    ...
                    if (this.camera3D.rotation.x > limit)
                        this.camera3D.rotation.x = limit;
                    if (this.camera3D.rotation.x < -limit)
                        this.camera3D.rotation.x = -limit;
                }
            }
        ...
        if (doc?.handlers?.update) {
            ...
            if (liveLayout) {
                ...
                if (cached && cached.width > 0 && cached.height > 0) {
                    ...
                    const my = this.input.getMouseY();
                    const baseMetricScale = this.getWorldsTextureScale();
                    const _liveMinUpd = Number.isFinite(this.worldsConfig.liveTextureScale)
                        ? Math.max(1, Math.min(4, this.worldsConfig.liveTextureScale))
                        : 2;
    ...
    render() {
        ...
        if (!doc.handlers) {
            if (this.frameCount < 3)
                console.log('🎨 No handlers object');
            return;
        }
        if (!doc.handlers.render) {
            if (this.frameCount < 3)
                console.log('🎨 No render handler');
            return;
        }
        try {
            doc.handlers.render();
    ...
    setupHiddenTextInputBridge() {
        if (typeof document === 'undefined' || this.hiddenTextInput)
            return;
    ...
    getWorldsSectionBoundGUIGroupIds(guiAPI) {
        const source = guiAPI ?? this.api?.gui;
        const bindings = Array.isArray(source?._sectionBindings)
    ...
    isPointOverVisibleGUIWidget(pixelX, pixelY, excludedGroups) {
        const guiAPI = this.api?.gui;
        const system = guiAPI?.getSystem?.();
        const manager = system?.getWidgetManager?.();
        let widgets = [];
        ...
        return widgets.some((widget) => {
            if (!widget?.state?.visible)
                return false;
            if (excludedGroups?.has(widget.group))
                return false;
    ...
    applyWorldsPositionConstraints(clampX, clampY, clampZ) {
        if (!this.camera3D)
            return;
        ...
        let overshootZ = 0;
        if (clampX) {
            if (nc.minX !== undefined && pos.x < nc.minX)
                overshootX = pos.x - nc.minX;
            if (nc.maxX !== undefined && pos.x > nc.maxX)
                overshootX = pos.x - nc.maxX;
        }
        if (clampY) {
            if (nc.minY !== undefined && pos.y < nc.minY)
                overshootY = pos.y - nc.minY;
            if (nc.maxY !== undefined && pos.y > nc.maxY)
                overshootY = pos.y - nc.maxY;
        }
        if (clampZ) {
            if (nc.minZ !== undefined && pos.z < nc.minZ)
                overshootZ = pos.z - nc.minZ;
            if (nc.maxZ !== undefined && pos.z > nc.maxZ)
                overshootZ = pos.z - nc.maxZ;
        }
    ...
    clampDollyForZBounds(dolly, forwardZ) {
        const nc = this.worldsConfig.navigationConstraints;
        if (!nc || !this.camera3D)
            return dolly;
        if (Math.abs(forwardZ) < 1e-6)
            return dolly;
    ...
    applyWorldsCameraPanDelta(dx, dy) {
        ...
        const move = vec3Add(vec3Scale(basis.right, -effectiveDx * worldPerPixel), vec3Scale(basis.up, effectiveDy * worldPerPixel));
    ...
    handleWorldsMiddlePanMove(pixelX, pixelY) {
        if (!this.middlePanActive || !this.worldsEnabled || !this.camera3D)
            return false;
    ...
    handleWorldsWheelEvent(e) {
        ...
        if (e.deltaMode === WheelEvent.DOM_DELTA_LINE)
            ...
            deltaY *= Math.max(1, this.canvas.height);
        ...
        this.applyWorldsCameraTranslation(basis.forward.x * dolly, basis.forward.y * dolly, basis.forward.z * dolly);
        return true;
    }
    startFreeFlyLeftDrag(pixelX, pixelY) {
        if (!this.worldsEnabled || !this.camera3D || !this.worldsControlsEnabled)
            return false;
        if (this.isPointOverVisibleGUIWidget(pixelX, pixelY))
            return false;
    ...
    handleFreeFlyLeftDragMove(pixelX, pixelY) {
        if (!this.worldsEnabled || !this.camera3D || !this.worldsControlsEnabled)
            return false;
        if (!this.freeFlyLeftPanActive && this.freeFlyLeftDragSectionIndex === null)
            return false;
    ...
    handleHiddenTextInputValueChange() {
        if (this.hiddenTextInputSyncing)
            return;
    ...
    runOrQueueGestureAudioStart(start) {
        const ctx = this.audioContext;
        if (this.trustedAudioGestureDepth > 0 ||
    ...
    handleWorldsMultiTouchRotate(e) {
        ...
        if (count === 3) {
            ...
            if (Number.isFinite(dy) && dy !== 0) {
                ...
                if (this.camera3D.rotation.x > pitchLimit)
                    this.camera3D.rotation.x = pitchLimit;
                if (this.camera3D.rotation.x < -pitchLimit)
                    this.camera3D.rotation.x = -pitchLimit;
            }
        }
    ...
    handleTouchMoveEvent(e) {
        ...
        if (this.worldsEnabled || dispatchedToDoc)
            e.preventDefault();
    }
    handleTouchEvent(e, action) {
        ...
        try {
            ...
            if (!this.worldsControlsEnabled && !inlineWidgetConsumed && !overlayGUIConsumed && !sectionGuiConsumed && action === 'press') {
                ...
                if (picked && this.camera3D) {
                    ...
                    if (linkHit) {
                        ...
                        this.activate3DLink(linkHit.region, picked.layout.sectionId, picked.layout.sectionIndex, linkHit.linkIndex);
                    }
            ...
            if (handledBy3D || dispatchedToDoc || this.worldsEnabled)
                e.preventDefault();
    ...
    async handleDroppedFile(file) {
        ...
        if (Number.isFinite(this.maxDropBytes) && file.size > this.maxDropBytes) {
            console.warn(`[drop] Ignoring dropped file "${file.name}" (${file.size} bytes) over maxDropBytes=${this.maxDropBytes}`);
            return;
        }
    ...
    installDropHandling(element = document.body) {
        if (this.dropHandlingCleanup)
            return this.dropHandlingCleanup;
        const prevent = (e) => {
            e.preventDefault();
            e.stopPropagation();
        };
        ...
        const cleanup = () => {
            ...
            if (this.dropHandlingCleanup === cleanup)
                this.dropHandlingCleanup = null;
        };
    ...
    handleKeyEvent(e, action) {
        ...
        try {
            ...
            if (!inlineWidgetHandled &&
                ...
                else if (this.worldsConfig.sectionArrowNavigation) {
    ...
    pick3DAt(pixelX, pixelY) {
        ...
        let best = null;
        for (const layout of this.section3DLayouts) {
            if (!layout.visible || !layout.texture || layout.interactive === false)
                continue;
    ...
    hitTest3DLinkAtUV(sectionIndex, u, v) {
        ...
        if (!dims || !regions || regions.length === 0)
            return null;
    ...
    getWorldsListMarkerGapPx() {
        ...
        return Number.isFinite(value) ? Math.max(0, Number(value)) : undefined;
    }
    ...
    activate3DLink(link, sectionId, sectionIndex, linkIndex) {
    ...
    getWorldsFocusRecenterOptions(keepRotation) {
        ...
        const hasDefaultRecenter = cfg.screenSpaceRecenter !== undefined;
    ...
    request3DCameraFrame(req) {
        ...
        if (!this.camera3D)
            return;
        const layouts = this.resolve3DCameraFrameLayouts(req.sectionSelectors, !!req.includeHidden, req.includeNonNavigable !== false);
        if (layouts.length === 0)
            return;
        ...
        const target = vec3Sub(center, vec3Scale(basis.forward, distance));
    ...
    premeasure3DCardWorldSize(layout) {
        ...
        if (layout.worldWidth && layout.worldHeight)
            return;
        const uiDocumentPack = this.getUIDocumentPack();
        if (!uiDocumentPack) {
            this.requestUIDocumentPack();
    ...
    getOutlineNodes() {
        ...
        if (!d)
            return [];
        if (this.outlineCache && this.outlineCache.documentId === d.id)
            return this.outlineCache.nodes;
        const nodes = [];
    ...
    applyWorldsLayoutCallback(sections) {
        ...
        const doc = sections
            ...
            : (runtimeSections.length > 0
                ...
                : (this.getActiveDocument()?.sections ? { sections: this.getActiveDocument().sections } : null));
        if (!doc?.sections)
            return;
        walk(doc.sections);
        for (let i = 0; i < Math.min(order.length, this.section3DLayouts.length); i++) {
            ...
            try {
                ...
                if (typeof out.width === 'number')
                    layout.width = out.width;
                if (typeof out.height === 'number')
                    layout.height = out.height;
                if (typeof out.opacity === 'number' && Number.isFinite(out.opacity)) {
                    layout.opacity = Math.max(0, Math.min(1, out.opacity));
                }
                if (typeof out.visible === 'boolean')
                    layout.visible = out.visible;
                if (typeof out.navigable === 'boolean')
                    layout.navigable = out.navigable;
                if (typeof out.interactive === 'boolean')
                    layout.interactive = out.interactive;
            }
            catch (error) {
    ...
    handleMouseMoveEvent(e) {
        ...
        const doc = this.getActiveDocument();
        if (!doc?.handlers?.input) {
            if (this.worldsEnabled || overlayGUIConsumed)
                e.preventDefault();
            return;
        }
