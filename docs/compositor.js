...
export class Compositor {
    ...
    pipelines = new Map();
    autoPipelines = new Map();
    sampler = null;
    vertexBuffer = null;
    uniformBuffer = null; // For blit parameters
    autoUniformBuffer = null; // For autoComposite opacity
    // Layers
    layers = new Map();
    // Mode
    mode = 'auto';
    ...
    autoClearValue = { r: 0, g: 0, b: 0, a: 1 };
    constructor(device, canvas) {
        ...
        if (!context)
            throw new Error('Failed to get WebGPU context for compositor');
    ...
    autoComposite() {
        ...
        const activeShaderName = (this.shaderManager && 'getActiveShaderName' in this.shaderManager && typeof this.shaderManager.getActiveShaderName === 'function')
            ? this.shaderManager.getActiveShaderName()
            : (this.shaderManager && 'getActiveShader' in this.shaderManager && typeof this.shaderManager.getActiveShader === 'function')
                ? this.shaderManager.getActiveShader()
                : null;
        ...
        if (shouldPostProcess) {
            ...
            if (this.shaderChainManager && this.shaderChainManager.hasActiveChain()) {
                try {
                    applied = this.shaderChainManager.applyChain(compositeTargetTexture, currentTexture, commandEncoder, this.materialTexture ?? undefined);
                }
                catch (error) {
            ...
            // Fall back to single shader if no chain
                ...
                try {
                    applied = this.shaderManager.applyShader(compositeTargetTexture, currentTexture, commandEncoder, this.materialTexture ?? undefined);
                }
                catch (error) {
    ...
    blit(layer, options = {}) {
        ...
        const transform = this.buildTransformMatrix(x, y, rotation, scaleX, scaleY, originX, originY);
    ...
    updateTextureFromCanvas(canvas, texture) {
        ...
        this.device.queue.copyExternalImageToTexture({ source: imageBitmap }, { texture }, { width: canvas.width, height: canvas.height });
        imageBitmap.close();
    }
    /**
