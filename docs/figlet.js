...
function parseIntAuto(s) {
    ...
    try {
        if (/^0x/i.test(t))
            return Number.parseInt(t.substring(2), 16);
        if (/^0[0-7]+$/.test(t) && t.length > 1)
            return Number.parseInt(t.substring(1), 8);
...
export function parseFIGfont(fontText, name) {
...
export function renderFigletLines(font, text, _opts = {}) {
    const t = String(text ?? '');
...
export function renderFigletCharLines(font, ch) {
    const height = Math.max(0, font.height | 0);
...
export function measureFigletLinesWidth(lines) {
    let w = 0;
