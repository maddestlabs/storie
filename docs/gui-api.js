 ...
 */
export function createGUIAPI(getMetrics, getStyle, isTrustedUserInput, getPixelScale, getViewportRect, getSafeAreaInsets, getCurrentWorldSection, resolveWorldSectionSelector) {
    const normalizeAutoMode = (value) => {
        if (value === true || value === 'auto')
            return true;
        if (value === false || value === 'manual' || value === undefined || value === null)
            return false;
        return !!value;
    };
    ...
    const safeGetScale = () => {
        try {
            if (typeof getPixelScale === 'function') {
                ...
                const sx = Number(s?.scaleX);
                const sy = Number(s?.scaleY);
    ...
    const scaleTokenPatch = (patch) => {
        ...
        if (patch.spacing) {
            next.spacing = {};
            if (typeof patch.spacing.xs === 'number')
                next.spacing.xs = scaleLength(patch.spacing.xs);
            if (typeof patch.spacing.sm === 'number')
                next.spacing.sm = scaleLength(patch.spacing.sm);
            if (typeof patch.spacing.md === 'number')
                next.spacing.md = scaleLength(patch.spacing.md);
            if (typeof patch.spacing.lg === 'number')
                next.spacing.lg = scaleLength(patch.spacing.lg);
            if (typeof patch.spacing.xl === 'number')
                next.spacing.xl = scaleLength(patch.spacing.xl);
        }
        if (patch.typography) {
            next.typography = {};
    ...
    const normalizeViewport = (viewport) => {
        if (!viewport || typeof viewport !== 'object')
            return viewport;
        const next = { ...viewport };
        const space = viewport.boundsSpace === 'device' || api._boundsSpace === 'device'
            ? 'device'
            : 'css';
        if (space === 'css') {
            ...
            if (typeof next.inset === 'number')
                next.inset = scaleLength(next.inset);
            if (typeof next.insetX === 'number')
                next.insetX = scaleLength(next.insetX);
            if (typeof next.insetY === 'number')
                next.insetY = scaleLength(next.insetY);
            if (typeof next.insetTop === 'number')
                next.insetTop = scaleLength(next.insetTop);
            if (typeof next.insetRight === 'number')
                next.insetRight = scaleLength(next.insetRight);
            if (typeof next.insetBottom === 'number')
                next.insetBottom = scaleLength(next.insetBottom);
            if (typeof next.insetLeft === 'number')
                next.insetLeft = scaleLength(next.insetLeft);
            if (typeof next.maxWidth === 'number')
                next.maxWidth = scaleLength(next.maxWidth);
            if (typeof next.maxHeight === 'number')
                next.maxHeight = scaleLength(next.maxHeight);
            if (typeof next.width === 'number' && typeof viewport.width === 'number')
                next.width = scaleLength(viewport.width);
            if (typeof next.height === 'number' && typeof viewport.height === 'number')
                next.height = scaleLength(viewport.height);
        }
        return next;
    };
    ...
    const toBreakpoint = (width, thresholds) => {
        ...
        if (!Number.isFinite(width) || width < t.sm)
            return 'xs';
        if (width < t.md)
            return 'sm';
        if (width < t.lg)
    ...
    const normalizeSectionList = (selector) => {
        ...
        for (const value of values) {
            ...
            if (section === null || resolved.includes(section))
                continue;
    ...
    const api = {
        ...
        init(options) {
            ...
            this._autoInput = normalizeAutoMode(options?.autoInput ?? options?.input);
            this._autoUpdate = normalizeAutoMode(options?.autoUpdate ?? options?.update);
        ...
        _normalizeConfig(config) {
            if (!config || typeof config !== 'object')
                return config;
            const space = config.boundsSpace === 'device' || this._boundsSpace === 'device'
                ? 'device'
                : 'css';
            if (space === 'device') {
                return config;
            ...
            const next = { ...config };
            if (next.bounds)
                next.bounds = scaleBounds(next.bounds);
            // Layout helpers also use pixel lengths.
            if (typeof next.padding === 'number')
                next.padding = scaleLength(next.padding);
            if (typeof next.gap === 'number')
                next.gap = scaleLength(next.gap);
            if (typeof next.rowGap === 'number')
                next.rowGap = scaleLength(next.rowGap);
            if (typeof next.columnGap === 'number')
                next.columnGap = scaleLength(next.columnGap);
            if (typeof next.maxWidth === 'number')
                next.maxWidth = scaleLength(next.maxWidth);
            if (typeof next.maxHeight === 'number')
                next.maxHeight = scaleLength(next.maxHeight);
            if (next.pianoStyle && typeof next.pianoStyle === 'object') {
                ...
                if (typeof next.pianoStyle.borderWidth === 'number')
                    next.pianoStyle.borderWidth = scaleLength(next.pianoStyle.borderWidth);
                if (typeof next.pianoStyle.railThickness === 'number')
                    next.pianoStyle.railThickness = scaleLength(next.pianoStyle.railThickness);
                if (typeof next.pianoStyle.railPadding === 'number')
                    next.pianoStyle.railPadding = scaleLength(next.pianoStyle.railPadding);
                if (typeof next.pianoStyle.labelInset === 'number')
                    next.pianoStyle.labelInset = scaleLength(next.pianoStyle.labelInset);
                if (typeof next.pianoStyle.minThumbLength === 'number')
                    next.pianoStyle.minThumbLength = scaleLength(next.pianoStyle.minThumbLength);
            }
            return next;
        },
        /**
        ...
        _resolveWidget(target) {
            if (!this._system || target === null || target === undefined)
                return null;
        ...
        _inferBindingMode(widget) {
            if (!widget)
                return null;
            if (typeof widget.getValue === 'function' && typeof widget.setValue === 'function')
                return 'value';
            if (typeof widget.isChecked === 'function' && typeof widget.setChecked === 'function')
                return 'checked';
            if (typeof widget.setText === 'function' || typeof widget.setLabel === 'function')
                return 'text';
            return null;
        },
        ...
        _syncScreenLayouts() {
            for (const entry of this._screenLayouts) {
                if (!entry?.container || typeof entry.container.fitToViewport !== 'function')
                    continue;
        ...
        screen(options) {
            ...
            if (layoutConfig) {
                ...
                const rootBounds = {
                    ...
                    width: Number(rawBounds?.width ?? layoutConfig.maxWidth ?? 640),
            ...
            const panel = {
                ...
                fitToViewport: (viewport, options, relayout = true) => {
                    ...
                    if (api._boundsSpace !== 'device') {
                        ...
                        const oo = { ...o };
                        const scaleOpt = (key) => {
                            if (!Object.prototype.hasOwnProperty.call(oo, key))
                                return;
...
//# sourceMappingURL=gui-api.js.map