...
This output still expects the host to provide a compatible Storie API object. The bundled raw document payload is included so a host can reconstruct asset stores while executing compiled behavior.
The next step is replacing that broad API with narrower capability-pack imports.
